...
export class ApiClient {
    ...
    async getRecommendedStartups(): Promise<StartupProfile[]> {
        try {
            return await this.requestWithCache('/investors/recommended-startups', {}, 3 * 60 * 1000); // 3 min cache
        } catch (error: any) {
            // Preserve the original error handling logic
            if (error.response?.status === 400) {
                const errorMessage = "You need to have an investor profile to see recommended startups. Please complete your investor profile first.";
                const newError: any = new Error(errorMessage);
                newError.response = error.response;
                throw newError;
            }
